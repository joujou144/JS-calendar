...
modeSwitch.addEventListener("click", lightMode);
...
function lightMode() {
   ...
   // check if the container already has a light class then replace it with dark class
   if (mainContainer.classList.contains("light")) {
      mainContainer.classList.replace("light", "dark");
      modeSwitchText.textContent = "dark mode"
   }
   // otherwise add the light class
   else {
      mainContainer.classList.add("light");
      modeSwitchText.textContent = "light mode"
